...
export default new Event('guildMemberRemove', async member => {
    ...
    // === NAGŁÓWEK „Pozostało #” ===
    const headerText = `Pozostało #${member.guild.memberCount - 1}`;
    ...
    ctx.fillText(`Żegnaj ${member.user.username}!`, canvas.width / 2, avatarY + 160);

    ctx.font = '22px Poppins';
    ctx.fillStyle = '#CCCCCC';
    ctx.fillText('do zobaczenia na', canvas.width / 2, avatarY + 190);

    ctx.font = 'bold 24px Poppins';
    ctx.fillStyle = '#a30909ff';
    ...
    logChannel.send({
        embeds: [
            new EmbedBuilder()
                .setColor('Red')
                .setDescription(`## Członek opuścił serwer\n> <@${member.user.id}> (${member.user.tag})`)
                .setFooter({
